    ...
    <div className="bg-gray-50 text-gray-900">
      ...
      <main className="md:ml-64 p-6 pt-24 transition-all min-h-screen">
        <Modal
          ...
          title={editingUser ? "Edit User" : `Add New ${defaultRole.charAt(0).toUpperCase() + defaultRole.slice(1)}`}
        >
          <UserForm
            ...
            <div className="relative">
              ...
              {isAddDropdownOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg z-20">
                  <button onClick={() => handleOpenModalForCreate('intern')} className="w-full text-left block px-4 py-2 text-gray-700 hover:bg-gray-100">Add Intern</button>
                  <button onClick={() => handleOpenModalForCreate('staff')} className="w-full text-left block px-4 py-2 text-gray-700 hover:bg-gray-100">Add Staff</button>
                </div>
              )}
            </div>
            {/* [MODIFIKASI SELESAI] */}
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={user.id} className="border-b border-gray-200">
                  ...
                  <td className="px-6 py-4 font-bold text-gray-800">{user.role}</td>
                  <td className="px-6 py-4">
                    <img
                      src={user.photo && user.photo !== '-' ? `${import.meta.env.VITE_STORAGE_URL}/img/avt/${user.photo}` : `https://ui-avatars.com/api/?name=${user.name.replace(' ', '+')}&background=random&color=fff`}
                      alt="user-avatar"
                      className="w-12 h-12 rounded-full object-cover"
                    />
                  </td>
                  <td className="px-6 py-4 font-bold text-gray-800">{user.date}</td>
                  <td className="px-6 py-4">
