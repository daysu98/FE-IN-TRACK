...
export const EditUser = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="font-semibold text-xl mb-6">Edit User</h2>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <select name="role" value={form.role || ''} onChange={handleChange} className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg">
            <option value="">Select Role</option>
            <option value="admin">Admin</option>
            <option value="staff">Staff</option>
            <option value="intern">Intern</option>
          </select>
          {validationError.role && <span className="text-red-500 text-sm block mt-1">{validationError.role[0]}</span>}
        </div>
        ...
        <div className="flex gap-4 pt-4">
          ...
          <Link to="/users" className="bg-gray-400 text-white px-6 py-2 rounded-lg hover:bg-gray-500 transition">CANCEL</Link>
        </div>
      </form>
    </div>
  );
