...
export const DetailJadwalAdminPage = () => {
	...
	const fetchData = async (day) => {
		try {
			const response = await axios.get(
				`${import.meta.env.VITE_API_BASE_URL}/api/picket-tasks?day=${day}`,
				{
	...
	const handleAddTask = async (e) => {
		...
		try {
			await axios.post(
				`${import.meta.env.VITE_API_BASE_URL}/api/picket-tasks`,
				{
					day: selectedDay,
	...
	const handleDeleteTask = async (id) => {
		try {
			await axios.delete(
				`${import.meta.env.VITE_API_BASE_URL}/api/picket-tasks/${id}`,
				{
					headers: { Authorization: `Bearer ${Cookies.get("token")}` },
				}
			);
				...
				<div className="bg-white p-6 rounded-xl">
					...
					<div className="mb-4">
						<select
							...
							className="w-full md:w-1/3 p-2 border rounded-lg cursor-pointer"
						>
							{days.map((day) => (
					...
					<form onSubmit={handleAddTask} className="flex gap-2 mb-6">
						...
						<button
							type="submit"
							className="bg-gray-800 text-white px-4 py-2 rounded-lg font-semibold text-sm"
						>
							Add Task
						</button>
					</form>
